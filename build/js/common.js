head.ready(function() {
 ...
 //    });
	...
	//open popup in present section
	$('.js-present-overlay').on('click', function(event){
		$(this).fadeOut();
		$('.js-present-popup').fadeIn();
		$('.js-present-video')[0].src += "&autoplay=1";
		event.stopPropagation()

	});

	//close popup on clicking on overlay
	$('.js-present-video-overlay').on('click', function () {
		$(this).parent().fadeOut();
		var newSrc = $('.js-present-video')[0].src.replace('&autoplay=1', '');
		$('.js-present-video')[0].src = newSrc;
		$('.js-present-overlay').fadeIn()
	});

	//scrollTo in video section
	$('.js-add-scroll-to').on('click', function(){
			$('html, body').animate({scrollTop:$('#js-add-scroll-target').position().top}, 1000);
	});


	//show-hide popup
	$('.js-add-popup-trig').on('click', function () {
		$('.js-add-popup').fadeIn();
	});
	$('.js-add-close-popup').on('click', function () {
		$('.js-add-popup').fadeOut()
	});

	//init slider in who-have section
	$('.js-who-have-slider').slick({
		infinite: true,
		slidesToShow: 1,
		slidesToScroll: 1,
		dots: false,
	...
	$('.js-who-have-left').on('click', function () {
		$('.js-who-have-slider').slick('slickNext')
	});
	$('.js-who-have-right').on('click', function () {
		$('.js-who-have-slider').slick('slickPrev')
	});

	//open popup in return section
	$('.js-return-popup-trig').on('click', function () {
		$('.js-return-popup').fadeIn();
		$('.js-return-video')[0].src += "&autoplay=1";
	});

	//close popup on clicking on overlay
	$('.js-return-overlay').on('click', function () {
		$(this).parent().fadeOut();
		var newSrc = $('.js-return-video')[0].src.replace('&autoplay=1', '');
		$('.js-return-video')[0].src = newSrc;
	});

	//open video in links
	$('.js-links-article').on('click', function () {
		var videoSrc = $(this).data('video');
		$('.js-links-popup').fadeIn();
		$('.js-links-video').attr('src', videoSrc += "?enablejsapi=1&autoplay=1");
	});

	$('.js-links-overlay').on('click', function () {
		$(this).parent().fadeOut();
		var newSrc = $('.js-links-video')[0].src.replace('&autoplay=1', '');
		$('.js-links-video')[0].src = newSrc;
	});

	$('.js-included-overlay').on('click', function () {

		var videoSrc = $(this).data('video');
		$('.js-included-popup').fadeIn();
		$('.js-included-video').attr('src', videoSrc += "?enablejsapi=1&autoplay=1");
	});

	$('.js-included-popup-overlay').on('click', function () {
		$(this).parent().fadeOut();
		var newSrc = $('.js-included-video')[0].src.replace('&autoplay=1', '');
		$('.js-included-video')[0].src = newSrc;
		event.stopPropagation()
	});
